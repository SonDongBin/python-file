...
# Series -> class : 인덱스 배열 index, 데이커 값 배열 value
print(sr.index) # -> ['a', 'b', 'c']
print(sr.values) # -> [ 1 2 3 ]
print(type(sr.values))
print(sr[0:2]) # 1,2, 숫자 인덱스로 자료를 가져옴, 숫자는 시작포함, end 포함 안함
...
# 딕셔너리로 데이터프레임 생성 : 키가 칼럼으로
# 시리즈가 여러개 모여서 데이터 프레임이 됨
...
# 2차원 배열을 데이터프레임으로 생성, 인덱스와 갈럼을 지정
...
# 일부 인덱스명만 변경 df.rename( index= {기존 인덱스 : 새로운 인덱스, ..})
# 일부 칼럼명만 변경 df.rename( columns= {기존 칼럼 : 새로운 칼럼, ..})
...
df.sort_values(by='수학') # value 값으로 정렬
...
while True:
    ...
    for index, data in enumerate(score):
        students[index].append(int(data))
